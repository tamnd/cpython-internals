# The same two startups on a build that does not use the frozen copies
...
Why this needs the debug build: it needs a build configured with --with-pydebug, which is the one build that leaves the frozen copies switched off by default, so the same program reports a different default.
