...
Checks are a proxy for the promise, so the lessons themselves are run there too. Every notebook, every code cell, in order, in a fresh browser runtime each, on every pull request. Eleven of the twelve run start to finish today. The one that does not is T07, where a cell meant to print a `RecursionError` overflows the WebAssembly call stack instead and takes the runtime with it, which is issue 105 and is written down in the report rather than quietly skipped.
