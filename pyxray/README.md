...
`pyxray.glossary` is the definitions, as data. One entry per term, with the thing people reliably get wrong about it, and a citation where the definition rests on something in CPython. `GLOSSARY.md` at the root of the repository is generated from it and the test suite compares the two, and lesson builders call `lesson.term("code object")` to link into it, which fails at build time if nobody has defined that term.
