...
Five lessons in, you have seen a lot of disassembly listings. You have probably been reading them the way most people read a foreign language they half know: picking out the words you recognise and guessing the rest.

This lesson is about stopping the guessing.

{figure("where-we-are", "the eight stages of running Python, with the code object highlighted")}

Nothing new gets built here. T05 handed us a code object and described it from the outside. This is the lesson where you learn to read what is inside one, so that from T07 onwards you can look at a listing and know what the interpreter is about to do without running it.
...
The interpreter is a stack machine. There is no `add r1, r2, r3` anywhere in it. There is a stack, and instructions push values onto it and pop values off it, and that is the whole model.

That sounds limiting until you see how little you need. Here is `total = total + n`, one instruction at a time, with the stack drawn next to each one.

{figure("the-stack-rising", "the stack after each instruction of total = total + n")}

Two loads push, the addition pops two and pushes one, and the store pops the last one. Started empty, ended empty, and got two high in the middle. That number, the highest the stack ever got, is `co_stacksize`, and the interpreter uses it to work out how much room to reserve for the frame before it starts.
...
lesson.md(f"""
An instruction is two bytes. One byte is the opcode, which says what to do. One byte is the argument, which says what to do it to.
...
`_Py_CODEUNIT` is a union. Read it one way and it is a 16 bit number, read it another way and it is the two fields. Fixed width instructions are why the interpreter can step forward without decoding anything first, and why every offset you see in a listing is even.
...
Here is the thing that stops most people the first time. `LOAD_CONST 1` and `LOAD_NAME 1` and `CALL 1` all print the same way, and the 1 means three unrelated things.
...
A few instructions do two things at once, and they fit two arguments into the one byte by using four bits each. It looks like a mistake in the listing the first time you see it: `LOAD_FAST_LOAD_FAST 18` loads slot 1 and slot 2, and 18 is neither of those.
...
One byte holds 0 to 255. Plenty of files have more than 256 names in them, and plenty of jumps are longer than 255 instructions. The way out is an instruction whose only job is to carry the bits that did not fit.

{figure("extended-arg", "four real bytes from a file with 300 names in it")}

`EXTENDED_ARG` shifts what it is holding left by eight and adds the next instruction's argument on: {cite("Python/bytecodes.c:6092-6098@v3.15.0rc1#EXTENDED_ARG")}. Up to three of them can stack in front of one instruction, which gets you a 32 bit argument, and that limit is stated in the comment on `_Py_CODEUNIT` above.
...
This is also the answer to a question that comes up when people first meet `dis`. Offsets in a listing are the offsets of the real instruction, and an `EXTENDED_ARG` in front of it occupies two of them, so the instruction you are looking at does not start where you expected. `dis` shows the `EXTENDED_ARG` rows, so you can always account for the difference.
...
Now the part that catches everybody out. A jump argument is not a byte offset and it is not an absolute address. It is a count of instructions, forwards or backwards, from where the interpreter would have gone next.

{figure("counting-a-jump", "the JUMP_BACKWARD at the end of a loop, and where the 14 comes from")}

Two separate adjustments are hiding in that. The count is in instructions and an instruction is two bytes, so multiply by two. And it counts from after the jump including the jump's own cache slot, not from the jump itself.
...
One thing worth knowing for anybody who learned this on an older Python. Absolute jumps are gone. Every jump in 3.15 is relative, and you can check that yourself rather than taking my word for it.
""")
...
`co_stacksize` is the number the interpreter uses to reserve room for the frame. Getting it wrong in either direction is bad: too small and the interpreter writes past the end of the frame, too large and every call wastes memory.
...
The rule is in the comment at the top of the function: {cite("Python/flowgraph.c:815-852@v3.15.0rc1#calculate_stackdepth")}. Start at the entry with an empty stack. Follow every edge. When two paths reach the same instruction, keep the deeper one. The answer is the deepest the walk ever got.
...
Each instruction's effect comes from two generated tables, `_PyOpcode_num_popped` and `_PyOpcode_num_pushed`, declared at {cite("Include/internal/pycore_opcode_metadata.h:35-38@v3.15.0rc1#_PyOpcode_num_popped")} and filled in from the stack signatures written on each instruction in `Python/bytecodes.c`. The compiler reads them through {cite("Python/flowgraph.c:772-798@v3.15.0rc1#get_stack_effects")}, and Python exposes the same numbers as `dis.stack_effect`.
""")
...
`pyxray.stack` is that walk, reimplemented in Python, in about forty lines. It is short enough to read in one sitting and it is the best way I know to be sure you have understood the rule.

One thing in it is not obvious from the description above. A `try` block has entry points that no instruction jumps to. The interpreter reaches them by looking up the exception table, so the walk has to seed them separately, and it has to seed them at the right height. The table is a list of five tuples: {cite("InternalDocs/exception_handling.md:108-118@v3.15.0rc1#exception")}. The height we want is the recorded depth, plus one if the handler asked for the instruction pointer to be pushed, plus one for the exception itself. That last plus one is easy to miss, and missing it is how the first version of this came out one short on 847 code objects.
...
A lesson can tell you a rule is right. It is better to check.
...
lesson.md("""
It found no disagreements at all. Run it over the whole standard library rather than the first 150 files and it still finds none, across thirty three thousand code objects.

That is what this project means when the front page says you could reimplement CPython from these lessons. The rule has been written down twice, once in C by the people who maintain the compiler and once in Python here, and the two agree everywhere anybody has looked.
...
This is a small thing and it is the kind of small thing that matters if you ever write a Python implementation of your own. The rule is not "the highest the stack gets". It is "the highest the stack gets, or one, whichever is more".

## Reading a listing cold

Time to put it together. Here is a function you have not seen, disassembled, and the source is below in a cell you can run afterwards. Try to work out what it does first.

{figure("reading-a-listing-cold", "the four questions, in the order that works")}

The order matters. Nobody reads a disassembly top to bottom. You look at what gets loaded, then what happens to it, then where it ends up, and only then do you sort out the jumps.
""")
...
Offsets skip numbers for two reasons: `EXTENDED_ARG` rows in front, and inline cache slots behind. Both are real bytes in `co_code`.
