...
STARTUP = Group(
    ...
    (
        ...
        Term(
            name="module spec",
            short="The answer a finder gives back, describing how a module is to be loaded.",
            long="A `ModuleSpec` carries the module's full name, the loader that will run its body, an origin string saying where it came from, and `submodule_search_locations`, which is what makes a module a package. Every module you can reach has one on its `__spec__` attribute, so you can ask any module in a running program which of the finders answered for it and what it loaded from.",
            cite="Lib/importlib/_bootstrap.py:605-640@v3.15.0rc1#ModuleSpec",
            also=("`ModuleSpec`", "`__spec__`", "`spec.origin`"),
            see=("meta path finder", "path entry finder"),
            met="R03",
        ),
        Term(
            name="meta path finder",
            short="Anything on sys.meta_path with a find_spec method, asked in order until one answers.",
            long="The import machinery hands each finder the module name and, for a submodule, the parent package's `__path__`, and takes the first spec it gets back. A fresh interpreter starts with three: `BuiltinImporter` for modules compiled into the binary, `FrozenImporter` for modules baked in as bytecode, and `PathFinder` for everything reachable from `sys.path`. The list is an ordinary list and you can put your own class on the front of it.",
            cite="Lib/importlib/_bootstrap.py:1198-1223@v3.15.0rc1#_find_spec",
            also=("`sys.meta_path`", "`find_spec`", "`_find_spec`"),
            see=("module spec", "path entry finder", "module lock"),
            met="R03",
        ),
        Term(
            name="path entry finder",
            short="A finder that owns one entry of sys.path, cached in sys.path_importer_cache.",
            long="`PathFinder` does not search directories itself. For each entry on `sys.path` it looks in `sys.path_importer_cache`, and on a miss it runs the entry past every hook in `sys.path_hooks` until one accepts it, storing the result, including `None` when nothing accepted. The usual answer is a `FileFinder`, which lists its directory once and reuses that listing until the directory's modification time changes.",
            cite="Lib/importlib/_bootstrap_external.py:1211-1231@v3.15.0rc1#_path_importer_cache",
            also=("`sys.path_importer_cache`", "`sys.path_hooks`", "`FileFinder`"),
            see=("meta path finder", "module spec"),
            met="R03",
        ),
        Term(
            name="module lock",
            short="One lock per module name, not one lock for importing.",
            long="Two threads importing two different modules do not wait for each other, because each takes a lock keyed on the name it is loading. Two threads importing the same name do wait, and only one of them runs the body. The lock is taken after a check of `sys.modules` that runs without it, so the common case of importing something already loaded costs a dict lookup and never touches a lock at all.",
            cite="Lib/importlib/_bootstrap.py:226-240@v3.15.0rc1#_ModuleLock",
            also=("`_ModuleLock`", "`_ModuleLockManager`", "`_blocking_on`"),
            see=("meta path finder", "interpreter state"),
            met="R03",
        ),
        Term(
            name="frozen module",
            short="A module whose bytecode was compiled during CPython's own build and written into the binary.",
            long="The build step compiles the source, marshals the code object and writes the bytes out as a C array in a generated header, so loading one is a read out of memory that was already mapped and no file is opened at all. Thirty three names are frozen in a stock 3.15 build. Three of them are the import system and cannot be switched off, and the rest are a startup speed decision that `-X frozen_modules=off` undoes.",
            cite="Python/frozen.c:74-79@v3.15.0rc1#bootstrap_modules",
            also=("`_imp.is_frozen`", "`-X frozen_modules`", "`Python/frozen.c`"),
            see=("import bootstrap", "loader state", "marshal"),
            met="R04",
        ),
        Term(
            name="import bootstrap",
            short="The C code that loads the Python import system before there is an import system.",
            ...
            cite="Python/import.c:3389-3428@v3.15.0rc1#init_importlib",
            also=("`init_importlib`", "`_setup`", "`_install`"),
            see=("frozen module", "meta path finder"),
            met="R04",
        ),
        Term(
            name="loader state",
            short="A free field on a module spec that a loader can use to remember whatever it likes.",
            long="`ModuleSpec` sets `loader_state` to `None` and never looks at it again, so the field belongs to whichever loader filled the spec in. `FrozenImporter` puts a small namespace there holding the path the module was compiled from and the name it was frozen under, which is how `os.__file__` ends up pointing at a file that the import itself never opened.",
            cite="Lib/importlib/_bootstrap.py:1106-1133@v3.15.0rc1#find_spec",
            also=("`spec.loader_state`", "`origname`", "`_fix_up_module`"),
            see=("module spec", "frozen module"),
            met="R04",
        ),
        Term(
            name="module alias",
            short="A frozen entry that carries the name its source file really had.",
            ...
            cite="Python/frozen.c:127-137@v3.15.0rc1#aliases",
            also=("`aliases`", "`is_alias`", "`__origname__`"),
...
BUILDING = Group(
    ...
    (
        ...
        Term(
            name="C API",
            short="The C functions and macros an extension may call to talk to the interpreter.",
            long="It is not one list but three, and which one you get is decided before you include anything. `Include/` is open to everybody, `Include/cpython/` needs `Py_LIMITED_API` to be undefined, and `Include/internal/` stops the compiler with an error unless you define `Py_BUILD_CORE`. More than half the header lines are in the third directory, the one no extension is meant to open.",
            cite="Doc/c-api/stable.rst:21-35@v3.15.0rc1",
            also=("`Python.h`",),
            see=("limited API", "PyAPI_FUNC", "stable ABI"),
            met="R06",
        ),
        Term(
            name="limited API",
            short="The subset of the C API you get when you define Py_LIMITED_API before Python.h.",
            long="It takes away about a quarter of the public functions, which is the part people notice, and all of the struct layouts, which is the part that costs. With no fields to read, `Py_TYPE` stops being a pointer dereference and becomes a function call, and `Py_DECREF` becomes a call to `_Py_DecRef`. What you buy with that is one built file that keeps working across releases.",
            cite="Include/object.h:124-126@v3.15.0rc1#_Py_OPAQUE_PYOBJECT",
            also=("`Py_LIMITED_API`",),
            see=("C API", "stable ABI"),
            met="R06",
        ),
        Term(
            name="PyAPI_FUNC",
            short="The macro that marks a declaration as a symbol other binaries may link against.",
            long="It expands to a compiler attribute asking for default visibility, so a declaration that uses it leaves the shared library and a plain `extern` does not. The internal headers use both spellings, often two lines apart, and the choice is deliberate: CPython's own bundled extensions are separate shared libraries that need to reach back in, and a comment above each export usually names which one.",
            cite="Include/exports.h:88-93@v3.15.0rc1#PyAPI_FUNC",
            see=("C API",),
            met="R06",
        ),
        Term(
            name="stable ABI",
            short="An extension build that keeps working across releases, marked abi3 in its name.",
            long="The tiers are a compile time idea and none of them survive into a built extension. The file name does. An interpreter decides what it is willing to load from the suffix alone, so `something.abi3.so` will load on many releases while `something.cpython-315-darwin.so` will load on exactly one. Python 3.15 adds `abi3t` for free threaded builds.",
            also=("abi3", "abi3t"),
            see=("limited API", "abi flags", "extension module"),
            met="R06",
        ),
        Term(
            name="extension module",
            short="A module that arrives as a compiled shared library rather than as Python.",
            long="Everything about how it loads is decided in two steps. First the finder looks at the file name and decides whether the tag in it is one this build accepts, which costs one stat call and happens before anything is opened. Then, if the extension was built against 3.15 or later, the interpreter compares the ABI struct it carries against itself, by which point the extension's own code has already run.",
            cite="Python/dynload_shlib.c:39-60@v3.15.0rc1#_PyImport_DynLoadFiletab",
            also=("`.so`", "`.pyd`"),
        ...
        Term(
            name="SOABI",
            short="The tag a build stamps into the file names of the extensions it compiles.",
