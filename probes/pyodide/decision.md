...
Twelve lessons, 180 code cells, and all twelve run start to finish.

One of them had to be changed to get there, and it is worth reading before writing any more Tier 0 material. T07 has a cell that recurses through `sorted` until the C stack runs out, which is meant to print a `RecursionError`. In WebAssembly the runtime overflows its own call stack first. That is not a Python exception and there is nothing to catch: the runtime does not come back, and in a notebook the reader loses the tab. The cell now checks `sys.platform` and prints a line saying it is not running that half here, which is a better lesson anyway, because the reason it cannot run is the thing the section is about. Issue 105.

A cell that takes the runtime down takes the rest of its lesson with it. Those cells are recorded as never reached rather than retried in a fresh runtime, because a cell that ran in a different interpreter than the cells above it has not been tested, it has been let off.
