# Nine ways a process can end, and what each one still runs on the way out
...
Why this needs the release build: it needs to start child interpreters, because the only honest way to watch a shutdown is from a process that outlives it.
