...
**A call that is the last thing a function does, so it can reuse the stack frame.**

Nothing happens after the call returns, so there is nothing left to come back for, and the compiler can jump to the callee rather than stack a new frame on top. CPython asks for this with `musttail`, which is a demand rather than a hint: if the compiler cannot manage it, the build fails instead of producing an interpreter that overflows the stack.

Also written musttail. First met in E11. See also [dispatch table](#dispatch-table), [calling convention](#calling-convention), [dispatch](#dispatch), [C stack](#c-stack). In the source: [`Python/ceval_macros.h:82-97@v3.15.0rc1`](https://github.com/python/cpython/blob/v3.15.0rc1/Python/ceval_macros.h#L82-L97).

### dispatch table

**An array of 256 entries, one per possible opcode byte, holding where to go next.**

In a computed goto build the entries are label addresses and in a tail calling build they are function pointers, but either way there is one entry for every value a byte can hold. Values that no opcode claimed point at a handler that reports a corrupt code object, which is why dispatch needs no bounds check.

First met in E11. See also [dispatch](#dispatch), [computed goto](#computed-goto), [tail call](#tail-call), [opcode](#opcode). In the source: [`Tools/cases_generator/target_generator.py:74-82@v3.15.0rc1`](https://github.com/python/cpython/blob/v3.15.0rc1/Tools/cases_generator/target_generator.py#L74-L82).

### calling convention

**The agreement about which registers a call may use and which it must hand back.**

It is normally invisible, because both sides of every call agree on it. The tail calling interpreter changes it on purpose with `preserve_none`, which says no register has to be handed back untouched, because the caller is finished and nobody is coming back to look. That is what lets the values being carried between opcodes stay in registers.

Also written preserve_none. First met in E11. See also [tail call](#tail-call), [C stack](#c-stack). In the source: [`Python/ceval_macros.h:74-80@v3.15.0rc1`](https://github.com/python/cpython/blob/v3.15.0rc1/Python/ceval_macros.h#L74-L80).
...
**One of the 32 block sizes obmalloc serves, sixteen bytes apart.**

A request is rounded up to the next multiple of sixteen and served from a pool holding only that size, so 16, 32, 48 and so on up to 512. Nothing in between exists. Asking for 17 bytes gets you 32 and the other fifteen are not given back to anyone until you free the block.

First met in M02. See also [block](#block), [pool](#pool), [obmalloc](#obmalloc). In the source: [`Include/internal/pycore_obmalloc.h:137-146@v3.15.0rc1#INDEX2SIZE`](https://github.com/python/cpython/blob/v3.15.0rc1/Include/internal/pycore_obmalloc.h#L137-L146).

### pool header

**The 48 bytes at the front of every pool that say what the pool is holding.**

It records how many blocks are in use, where the free list starts, which size class the pool serves, which arena it belongs to, and the two pointers linking it to the other pools of the same class. Those 48 bytes are why a pool holds 1021 sixteen byte blocks rather than 1024.

First met in M02. See also [pool](#pool), [block](#block), [arena](#arena). In the source: [`Include/internal/pycore_obmalloc.h:263-274@v3.15.0rc1#pool_header`](https://github.com/python/cpython/blob/v3.15.0rc1/Include/internal/pycore_obmalloc.h#L263-L274).

### mimalloc

**The second small object allocator, already compiled into your Python and picked with one variable.**

It came from Microsoft Research and CPython vendors a modified copy of it. Setting `PYTHONMALLOC=mimalloc` swaps it in for obmalloc on any ordinary build, and the free threaded build has no choice about it because obmalloc is compiled out there entirely. It serves requests up to 16384 bytes across 73 size classes rather than 512 bytes across 32.

First met in M03. See also [obmalloc](#obmalloc), [segment](#segment), [mimalloc heap](#mimalloc-heap), [PYTHONMALLOC](#pythonmalloc). In the source: [`Objects/obmalloc.c:781-796@v3.15.0rc1#PYMEM_ALLOCATOR_MIMALLOC`](https://github.com/python/cpython/blob/v3.15.0rc1/Objects/obmalloc.c#L781-L796).

### segment

**mimalloc's version of an arena, 32 MiB asked from the system and split into 64 KiB pages.**

The record describing each page lives in the segment header rather than in the page itself, which is the one design decision the rest of mimalloc follows from. Because nothing is stored in front of the blocks, a block can start exactly at the page boundary, and a page can be handed to a single thread without anybody else needing to read it.

First met in M03. See also [mimalloc](#mimalloc), [arena](#arena), [pool](#pool). In the source: [`Include/internal/mimalloc/mimalloc/types.h:203-211@v3.15.0rc1#MI_SEGMENT_SHIFT`](https://github.com/python/cpython/blob/v3.15.0rc1/Include/internal/mimalloc/mimalloc/types.h#L203-L211).

### mimalloc heap

**One thread's private set of pages, of which each thread gets four rather than one.**

The four are mem for plain buffers, object for objects the cycle collector never has to look at, and two more for collected objects, split by whether the object carries a pre header. Keeping collected objects in their own heaps is what lets the free threaded collector find every object by walking the heap instead of by following a linked list.

First met in M03. See also [mimalloc](#mimalloc), [segment](#segment), [GC pre header](#gc-pre-header), [cycle collector](#cycle-collector). In the source: [`Include/internal/pycore_mimalloc.h:12-18@v3.15.0rc1#_Py_mimalloc_heap_id`](https://github.com/python/cpython/blob/v3.15.0rc1/Include/internal/pycore_mimalloc.h#L12-L18).

### allocator domain

**One of the three entrances to CPython's heap, each with its own set of functions.**

Raw is for work that happens outside an interpreter and has to be safe to call with no lock held, mem is for buffers that belong to an object, and obj is for the objects themselves. They are not interchangeable. Each domain is four function pointers a program embedding Python may replace, and under the debug hooks each one writes a different letter in front of every block it hands out.

Also written `PYMEM_DOMAIN_RAW`, `PYMEM_DOMAIN_MEM`, `PYMEM_DOMAIN_OBJ`. First met in M01. See also [debug hooks](#debug-hooks), [obmalloc](#obmalloc), [PYTHONMALLOC](#pythonmalloc). In the source: [`Include/cpython/pymem.h:5-14@v3.15.0rc1#PyMemAllocatorDomain`](https://github.com/python/cpython/blob/v3.15.0rc1/Include/cpython/pymem.h#L5-L14).

### debug hooks

**A wrapper around whatever allocator is underneath, adding a header and fences to every block.**

Sixteen extra bytes go in front of your pointer, holding the size you asked for and one letter naming the domain, and eight more go behind it. The fences are filled with a byte that is not a plausible number or address, fresh memory is filled with another and freed memory with a third. Every free reads them all back and stops the process rather than continuing on a corrupted heap.

First met in M01. See also [allocator domain](#allocator-domain), [PYTHONMALLOC](#pythonmalloc). In the source: [`Objects/obmalloc.c:3088-3097@v3.15.0rc1`](https://github.com/python/cpython/blob/v3.15.0rc1/Objects/obmalloc.c#L3088-L3097).

### PYTHONMALLOC

**The environment variable that picks what sits underneath the three domains.**

It is read once at startup and nothing above the domains can tell the difference afterwards. Setting it to `malloc` takes CPython's own small object allocator out entirely, which is what a tool like Valgrind wants because it needs to see each allocation rather than one large chunk being carved up out of sight. Adding `debug` to any of the names switches the fences on as well.

First met in M01. See also [allocator domain](#allocator-domain), [debug hooks](#debug-hooks), [obmalloc](#obmalloc). In the source: [`Include/cpython/pymem.h:16-30@v3.15.0rc1#PyMemAllocatorName`](https://github.com/python/cpython/blob/v3.15.0rc1/Include/cpython/pymem.h#L16-L30).

### ownership

**The rule about who is responsible for putting a reference back down.**
...
First met in M04. See also [reference count](#reference-count), [new reference](#new-reference), [borrowed reference](#borrowed-reference), [stolen reference](#stolen-reference). In the source: [`Include/refcount.h:285-292@v3.15.0rc1#ob_refcnt`](https://github.com/python/cpython/blob/v3.15.0rc1/Include/refcount.h#L285-L292).

### reentrancy

**Code being entered again while an earlier call to it is still part way through.**

Dropping the last reference to an object runs that object's destructor, and a destructor can be arbitrary Python, so any C code that drops a reference has to be finished making sense of itself first. This is why a container empties itself before it releases what it held, rather than the other way round.

First met in M04. See also [deallocation](#deallocation), [finalizer](#finalizer), [reference count](#reference-count). In the source: [`Objects/dictobject.c:3073-3098@v3.15.0rc1#clear_lock_held`](https://github.com/python/cpython/blob/v3.15.0rc1/Objects/dictobject.c#L3073-L3098).

### deferred reference counting

**Marking an object so that nothing counts references to it at all.**

The free threaded build sets an object's shared count to `PY_SSIZE_T_MAX / 8`, which is so far from zero that no amount of dropping references will ever get there. Nothing frees the object by counting, so only the cycle collector can, and it is the only thing that looks. Top level functions, classes, modules and methods get this, because they are read constantly from every thread and almost never dropped.

Also written `_Py_REF_DEFERRED`. First met in M06. See also [free threaded build](#free-threaded-build), [immortal object](#immortal-object), [reference count](#reference-count). In the source: [`Objects/object.c:2802-2812@v3.15.0rc1#_PyObject_SetDeferredRefcount`](https://github.com/python/cpython/blob/v3.15.0rc1/Objects/object.c#L2802-L2812).

### biased reference counting

**Splitting one count into a fast half for the owning thread and a shared half for everybody else.**
...
**Parking every other thread so the collector can look at a heap that holds still.**

Removing the GIL did not remove the collector's need for a moment when nothing is changing. A thread running Python gets a bit set on its eval breaker and parks itself between two bytecode instructions. A thread already blocked in C is marked parked where it stands and never woken. The collector waits in one millisecond steps until the last one has stopped.

Also written `_PyEval_StopTheWorld`, `_PY_EVAL_PLEASE_STOP_BIT`. First met in M08. See also [free threaded build](#free-threaded-build), [cycle collector](#cycle-collector). In the source: [`Python/pystate.c:2408-2443@v3.15.0rc1#stop_the_world`](https://github.com/python/cpython/blob/v3.15.0rc1/Python/pystate.c#L2408-L2443).

### mark alive pass

**A first sweep that finds what is obviously reachable so the real pass can skip it.**
...
Also written `gc_mark_alive_from_roots`, `_PyGC_BITS_ALIVE`. First met in M08. See also [cycle collector](#cycle-collector), [free threaded build](#free-threaded-build). In the source: [`Python/gc_free_threading.c:1376-1401@v3.15.0rc1#gc_mark_alive_from_roots`](https://github.com/python/cpython/blob/v3.15.0rc1/Python/gc_free_threading.c#L1376-L1401).

### referrer

**An object that points at the one you asked about, as far as tp_traverse can tell.**

`gc.get_referrers` finds them by walking every tracked object in every generation and calling its tp_traverse to see whether your object comes up. That makes it cost a walk of the whole heap, and it makes it blind to anything untracked, so a tuple of plain integers can be holding your object and never appear in the answer.

Also written `gc.get_referrers`, `gc_referrers_for`. First met in M09. See also [cycle collector](#cycle-collector), [reference cycle](#reference-cycle). In the source: [`Python/gc.c:1665-1684@v3.15.0rc1#gc_referrers_for`](https://github.com/python/cpython/blob/v3.15.0rc1/Python/gc.c#L1665-L1684).

### DEBUG_SAVEALL

**A debug flag that puts everything a pass would have freed into gc.garbage instead.**

Without it a collection calls tp_clear on each unreachable object and the objects go away, which is what you want and also means you never get to look at them. With it set, delete_garbage appends each one to the garbage list and clears nothing, so after a pass you have the exact set of objects that were only kept alive by each other.

Also written `gc.set_debug`, `gc.garbage`. First met in M09. See also [cycle collector](#cycle-collector), [reference cycle](#reference-cycle). In the source: [`Python/gc.c:1082-1110@v3.15.0rc1#delete_garbage`](https://github.com/python/cpython/blob/v3.15.0rc1/Python/gc.c#L1082-L1110).
...
The runtime does this to the strings it interns for names while a module is being compiled, and to the objects it sets up at startup. There is no way to ask for it from Python, which is deliberate, because there is no way to undo it either. An object that gets the treatment is never freed again for the life of the process.

First met in M05. See also [immortal object](#immortal-object), [interning](#interning), [reference count](#reference-count). In the source: [`Objects/unicodeobject.c:14196-14214@v3.15.0rc1#immortalize_interned`](https://github.com/python/cpython/blob/v3.15.0rc1/Objects/unicodeobject.c#L14196-L14214).

### single character cache

**The 256 one character strings the runtime builds once before your code runs.**

Every string of one byte comes out of this table rather than being built, which is why slicing or indexing a string of ASCII never allocates. They are immortal, so nothing ever counts references to them, and the table is one of the reasons a program that shuffles short strings around does far less allocation than it looks like it should.

Also written `_Py_LATIN1_CHR`. First met in M05. See also [small integer cache](#small-integer-cache), [immortal object](#immortal-object), [interning](#interning). In the source: [`Objects/unicodeobject.c:1809-1814@v3.15.0rc1#get_latin1_char`](https://github.com/python/cpython/blob/v3.15.0rc1/Objects/unicodeobject.c#L1809-L1814).
...
**One lock per interpreter, and you have to be holding it to run Python.**

The struct behind the name is six fields: a boolean saying whether anybody holds it, a mutex and a condition variable to wait on, the number of microseconds a waiting thread stays patient, which thread had it last, and a count of how many times it has changed hands. Everything surprising about threads in CPython comes out of when a running thread is willing to let go of it.

Also written global interpreter lock. First met in C01. See also [switch interval](#switch-interval), [eval breaker](#eval-breaker), [free threaded build](#free-threaded-build). In the source: [`Include/internal/pycore_gil.h:22-61@v3.15.0rc1#_gil_runtime_state`](https://github.com/python/cpython/blob/v3.15.0rc1/Include/internal/pycore_gil.h#L22-L61).

### switch interval

**How long a thread waiting for the GIL waits before it asks for it.**
...
Also written `sys.setswitchinterval`. First met in C01. See also [GIL](#gil), [eval breaker](#eval-breaker). In the source: [`Python/ceval_gil.c:147-153@v3.15.0rc1#DEFAULT_INTERVAL`](https://github.com/python/cpython/blob/v3.15.0rc1/Python/ceval_gil.c#L147-L153).

### eval breaker

**A word of bits on the thread state that the eval loop checks between instructions.**
...
Also written `tstate->eval_breaker`. First met in C01. See also [GIL](#gil), [periodic check](#periodic-check). In the source: [`Include/internal/pycore_ceval.h:345-352@v3.15.0rc1#_PY_GIL_DROP_REQUEST_BIT`](https://github.com/python/cpython/blob/v3.15.0rc1/Include/internal/pycore_ceval.h#L345-L352).

### periodic check

**The one place in the eval loop where a thread can be made to give up the GIL.**

Backward jumps and function resumes run a `_CHECK_PERIODIC` instruction, which reads the eval breaker and calls `_Py_HandlePending` if any bit is set. That is the only route from running code to releasing the lock, which is why a single long call into C cannot be interrupted no matter what the switch interval says.

Also written `_CHECK_PERIODIC`. First met in C01. See also [eval breaker](#eval-breaker), [GIL](#gil). In the source: [`Python/ceval_macros.h:520-528@v3.15.0rc1#check_periodics`](https://github.com/python/cpython/blob/v3.15.0rc1/Python/ceval_macros.h#L520-L528).

### race condition

**Two threads touching the same thing, where the answer depends on which got there first.**

The usual shape is a read, a change and a write back, with another thread reading the old value in between. Nothing about it is specific to Python, but on a build with the GIL a great many of them never happen, because the interpreter only hands the lock over at a periodic check and most short sequences do not contain one.

First met in C02. See also [periodic check](#periodic-check), [critical section](#critical-section), [per object lock](#per-object-lock). In the source: [`Python/ceval_macros.h:520-528@v3.15.0rc1#check_periodics`](https://github.com/python/cpython/blob/v3.15.0rc1/Python/ceval_macros.h#L520-L528).

### per object lock

**A one byte mutex sitting in every object header on the free threaded build.**

`PyMutex` uses two bits of one byte: whether it is held, and whether anybody is parked waiting for it. Taking a free one is a single compare and exchange with no system call, which is why it is cheap enough to put one in every object rather than in a table on the side.

Also written `ob_mutex`, `PyMutex`. First met in C02. See also [critical section](#critical-section), [free threaded build](#free-threaded-build), [object header](#object-header). In the source: [`Include/cpython/pylock.h:12-35@v3.15.0rc1#PyMutex`](https://github.com/python/cpython/blob/v3.15.0rc1/Include/cpython/pylock.h#L12-L35).

### critical section

**A block of C that holds an object's lock, and is allowed to let go of it partway.**

Written as `Py_BEGIN_CRITICAL_SECTION(op)` and `Py_END_CRITICAL_SECTION()`, and compiled to a bare pair of braces on any build that has the GIL. The letting go is the interesting part: an inner section suspends the outer ones rather than nesting inside them, which is what makes deadlock impossible without anybody having to order the locks.

Also written `Py_BEGIN_CRITICAL_SECTION`. First met in C02. See also [per object lock](#per-object-lock), [free threaded build](#free-threaded-build). In the source: [`Include/critical_section.h:74-90@v3.15.0rc1#Py_BEGIN_CRITICAL_SECTION`](https://github.com/python/cpython/blob/v3.15.0rc1/Include/critical_section.h#L74-L90).

### thread state

**The struct the interpreter keeps for one thread, holding everything only that thread has.**

A `PyThreadState` carries the frame the thread is running, the exception it is handling, how much recursion it has left, a scratch dict, the key its `threading.local` values hang off, and its place in the interpreter's list of threads. The operating system thread underneath it knows none of this.

Also written `PyThreadState`, `tstate`. First met in C03. See also [attach and detach](#attach-and-detach), [frame](#frame), [daemon thread](#daemon-thread). In the source: [`Include/cpython/pystate.h:66-101@v3.15.0rc1#_ts`](https://github.com/python/cpython/blob/v3.15.0rc1/Include/cpython/pystate.h#L66-L101).

### attach and detach

**A thread taking hold of its thread state before running Python, and letting go after.**

Attaching sets one int on the thread state and, on a build with the GIL, takes the lock on the way in. Detaching does the reverse and is what `Py_BEGIN_ALLOW_THREADS` expands to, which is why a C function that releases the GIL and a C function that detaches its thread state are the same thing said two ways.

Also written `PyEval_SaveThread`, `PyEval_RestoreThread`. First met in C03. See also [thread state](#thread-state), [GIL](#gil), [stop the world](#stop-the-world). In the source: [`Python/pystate.c:2225-2251@v3.15.0rc1#_PyThreadState_Attach`](https://github.com/python/cpython/blob/v3.15.0rc1/Python/pystate.c#L2225-L2251).

### daemon thread

**A thread the interpreter will not wait for, and will hang wherever it stands at exit.**

Shutdown does not ask a daemon thread to stop. It writes one value into that thread's thread state, and the thread carries on until its next periodic check, tries to attach, reads the value and is parked forever. No `finally` block runs and no `with` block exits, which is why a daemon thread should never be the only thing holding a file open.

Also written `daemon=True`. First met in C03. See also [thread state](#thread-state), [attach and detach](#attach-and-detach), [periodic check](#periodic-check). In the source: [`Python/pystate.c:3199-3212@v3.15.0rc1#_PyThreadState_HangThread`](https://github.com/python/cpython/blob/v3.15.0rc1/Python/pystate.c#L3199-L3212).

### runtime state

**The one struct per process, holding everything that sits above a single interpreter.**

`_PyRuntime` is a plain global variable rather than a pointer, so it exists before anything has been set up. It holds the list of interpreters, the process wide table of signal handlers, the objects every interpreter shares, and the field offsets an out of process debugger needs in order to read the rest. There is exactly one and you cannot make another.

Also written `_PyRuntimeState`, `_PyRuntime`. First met in R02. See also [interpreter state](#interpreter-state), [thread state](#thread-state), [static object](#static-object). In the source: [`Include/internal/pycore_runtime.h:14-22@v3.15.0rc1#_PyRuntime`](https://github.com/python/cpython/blob/v3.15.0rc1/Include/internal/pycore_runtime.h#L14-L22).

### interpreter state

**The struct holding one interpreter's modules, its heap, its threads and its lock.**

A `PyInterpreterState` is one level up from a thread state. It owns the list of threads belonging to it, its own `sys.modules`, its own allocator arenas and, since PEP 684, its own GIL. One process can hold several of them, chained newest first off a list hanging on the runtime.

Also written `PyInterpreterState`, `interp`. First met in C04. See also [subinterpreter](#subinterpreter), [thread state](#thread-state), [GIL](#gil). In the source: [`Include/internal/pycore_runtime_structs.h:184-201@v3.15.0rc1#pyinterpreters`](https://github.com/python/cpython/blob/v3.15.0rc1/Include/internal/pycore_runtime_structs.h#L184-L201).

### subinterpreter

**A second interpreter inside the same process, with its own modules and its own lock.**

Made from Python with `concurrent.interpreters.create()` and from C with `Py_NewInterpreterFromConfig`. It shares the process, the address space and the immortal objects, and almost nothing else. Because it has a lock of its own, work running in one is not held up by work running in another.

Also written `concurrent.interpreters`, PEP 734. First met in C04. See also [interpreter state](#interpreter-state), [GIL](#gil), [shareable object](#shareable-object). In the source: [`Include/cpython/pylifecycle.h:40-64@v3.15.0rc1#_PyInterpreterConfig_INIT`](https://github.com/python/cpython/blob/v3.15.0rc1/Include/cpython/pylifecycle.h#L40-L64).

### shareable object

**An object the runtime knows how to hand from one interpreter to another.**

Seven types are registered for it: `None`, `int`, `bytes`, `str`, `bool`, `float` and `tuple`. Anything else has to be turned into bytes on the way in and built again on the way out, which means what arrives is a copy, so changing it on one side is invisible on the other.

Also written `is_shareable`, `concurrent.interpreters.Queue`. First met in C04. See also [subinterpreter](#subinterpreter), [immortal object](#immortal-object). In the source: [`Python/crossinterp_data_lookup.h:790-829@v3.15.0rc1#_register_builtins_for_crossinterpreter_data`](https://github.com/python/cpython/blob/v3.15.0rc1/Python/crossinterp_data_lookup.h#L790-L829).

### static object

**An object built into the runtime struct itself, shared by every interpreter in the process.**

`_Py_static_objects` holds `None`, the small ints from minus five up to 1024, the empty bytes and the 256 one byte bytes objects, every one character latin-1 string and a table of fixed identifier strings. They are fields of the runtime rather than allocations, so `id()` gives the same answer whichever interpreter you ask, and they are immortal because there is no allocation to hand back.

Also written `_PY_NSMALLPOSINTS`, `_Py_global_strings`. First met in R02. See also [runtime state](#runtime-state), [immortal object](#immortal-object), [static type](#static-type). In the source: [`Include/internal/pycore_runtime_structs.h:102-126@v3.15.0rc1#_Py_static_objects`](https://github.com/python/cpython/blob/v3.15.0rc1/Include/internal/pycore_runtime_structs.h#L102-L126).

### pending call

**A C function left for a thread to run the next time it reaches a periodic check.**

The caller does not run anything. It puts a function pointer on a small ring buffer, sets a bit on the eval breaker, and returns. `Py_AddPendingCall` always leaves the work for the main thread of the main interpreter, which is how a signal arriving on any thread ends up being handled on the one thread that is allowed to handle it.

Also written `_PY_CALLS_TO_DO_BIT`, `_PyEval_AddPendingCall`. First met in C05. See also [eval breaker](#eval-breaker), [periodic check](#periodic-check). In the source: [`Python/ceval_gil.c:810-825@v3.15.0rc1#Py_AddPendingCall`](https://github.com/python/cpython/blob/v3.15.0rc1/Python/ceval_gil.c#L810-L825).

### asynchronous exception

**An exception set on one thread by another, raised when the target next looks.**

`PyThreadState_SetAsyncExc` finds the thread state with a matching id, stores the exception class on it and sets a bit. The target raises it at its next periodic check, so a thread spinning in Python stops almost at once and a thread inside one long C call carries on until that call returns.

Also written `tstate->async_exc`, `_PY_ASYNC_EXCEPTION_BIT`. First met in C05. See also [thread state](#thread-state), [periodic check](#periodic-check). In the source: [`Python/pystate.c:2544-2580@v3.15.0rc1#PyThreadState_SetAsyncExc`](https://github.com/python/cpython/blob/v3.15.0rc1/Python/pystate.c#L2544-L2580).

### signal handler

**A Python function the operating system cannot call, so the runtime calls it later.**

The C handler the kernel actually runs does almost nothing: it records which signal arrived and sets a bit on the main thread's eval breaker. The Python function registered with `signal.signal` runs from the periodic check afterwards, always on the main thread of the main interpreter, no matter which thread the operating system delivered the signal to.

Also written `signal.signal`, `_PyEval_SignalReceived`. First met in C05. See also [eval breaker](#eval-breaker), [pending call](#pending-call). In the source: [`Modules/signalmodule.c:80-101@v3.15.0rc1#signal`](https://github.com/python/cpython/blob/v3.15.0rc1/Modules/signalmodule.c#L80-L101).

### optimistic read

**Take a reference first, then check that what you took it on has not moved.**

`_Py_TryIncrefCompare` reads a pointer, tries to bump the count on whatever it found, and then reads the pointer a second time. If the two reads disagree the object was replaced underneath it, so it drops the reference and the caller starts again. No lock is taken on the way in, which is why four threads can read the same list at once.

Also written `_Py_TryIncrefCompare`, `_Py_TryXGetRef`. First met in C06. See also [critical section](#critical-section), [free threaded build](#free-threaded-build), [reference count](#reference-count). In the source: [`Include/internal/pycore_object.h:571-586@v3.15.0rc1#_Py_TryIncrefCompare`](https://github.com/python/cpython/blob/v3.15.0rc1/Include/internal/pycore_object.h#L571-L586).

### safe memory reclamation

**Freeing memory only once every thread has been seen to move on past it.**

Also written QSBR. A thread that replaces a container's storage cannot free the old block straight away, because another thread may still be reading it. So the block goes on a queue with a sequence number, and the memory is handed back later, once every thread has reached a point where it is certainly no longer looking. The draining happens at the periodic check, which is the same eval breaker C05 was about.

Also written QSBR, `_PyMem_FreeDelayed`. First met in C06. See also [optimistic read](#optimistic-read), [periodic check](#periodic-check), [free threaded build](#free-threaded-build). In the source: [`Include/internal/pycore_qsbr.h:1-30@v3.15.0rc1#QSBR`](https://github.com/python/cpython/blob/v3.15.0rc1/Include/internal/pycore_qsbr.h#L1-L30).

### reference count contention

**Several threads writing the same count, which is the slowest thing a CPU does.**
...
Also written cache line ping pong. First met in C06. See also [immortal object](#immortal-object), [biased reference counting](#biased-reference-counting), [free threaded build](#free-threaded-build).

### thread safety declaration

**A slot an extension module fills in to say it can run without the lock.**

A compiled module can add `Py_mod_gil` to its slot table with the value `Py_MOD_GIL_NOT_USED`. Saying nothing is not neutral, because a module written before free threading existed could not have said anything, so the runtime treats silence as a request for the lock and turns it back on when the module is imported.

Also written `Py_mod_gil`, `Py_MOD_GIL_NOT_USED`, `Py_MOD_GIL_USED`. First met in C07. See also [free threaded build](#free-threaded-build), [transient GIL](#transient-gil), [slot](#slot). In the source: [`Include/moduleobject.h:85-89@v3.15.0rc1#Py_MOD_GIL_NOT_USED`](https://github.com/python/cpython/blob/v3.15.0rc1/Include/moduleobject.h#L85-L89).

### transient GIL

**The lock switched on for the length of one import and then switched off again.**
...
Also written `_PyEval_EnableGILTransient`, `_PyEval_DisableGIL`, `gil->enabled`. First met in C07. See also [thread safety declaration](#thread-safety-declaration), [free threaded build](#free-threaded-build), [stop the world](#stop-the-world). In the source: [`Python/ceval_gil.c:1080-1114@v3.15.0rc1#_PyEval_EnableGILTransient`](https://github.com/python/cpython/blob/v3.15.0rc1/Python/ceval_gil.c#L1080-L1114).

### abi flags

**The letters in a build's file names that say which builds it can load.**

`sys.abiflags` is `t` on a free threaded build and empty on an ordinary one, and that letter goes into the file name of every compiled module the build can import. It is what stops a wheel built for one from being loaded into the other, since the two disagree about the shape of every object header.

Also written `sys.abiflags`, `EXT_SUFFIX`, abi3t. First met in C07. See also [free threaded build](#free-threaded-build), [thread safety declaration](#thread-safety-declaration).

### cross interpreter data

**The plain bytes an object is reduced to so a second interpreter can rebuild it.**

Written xidata in the source. Seven types have a direct route, and everything else falls back: first the object itself, then the function path for a callable that carries no state, then pickle. Whichever route is taken, what arrives on the other side is a new object at a new address, so nothing is shared and nothing can be changed from both sides.

Also written xidata, `_PyXIData_t`, `_PyObject_GetXIData`. First met in C08. See also [shareable object](#shareable-object), [subinterpreter](#subinterpreter), [stateless function](#stateless-function). In the source: [`Python/crossinterp.c:530-558@v3.15.0rc1#_PyObject_GetXIData`](https://github.com/python/cpython/blob/v3.15.0rc1/Python/crossinterp.c#L530-L558).

### stateless function

**A function that reads no globals and closes over nothing, so it can be sent whole.**

`Interpreter.call` can hand one of these to another interpreter by sending the code itself rather than a name to look up. A function that reads a global cannot go that way, because the name would have to be resolved on the other side, where the module it came from does not exist. It is the reason a recursive function written at the top level will not cross.

Also written `Interpreter.call`, `_PyFunction_GetXIData`. First met in C08. See also [cross interpreter data](#cross-interpreter-data), [subinterpreter](#subinterpreter). In the source: [`Python/crossinterp_data_lookup.h:755-780@v3.15.0rc1#_PyFunction_GetXIData`](https://github.com/python/cpython/blob/v3.15.0rc1/Python/crossinterp_data_lookup.h#L755-L780).

### interpreter pool

**A pool of workers where each worker is an interpreter rather than a thread.**

`InterpreterPoolExecutor` has the same shape as the thread and process pools, so `submit` and `map` work the way you already expect. The difference is what happens to the arguments and the return value, which have to be turned into bytes and rebuilt on each hop, so it is worth using only when a job does much more work than it carries.

Also written `concurrent.futures.InterpreterPoolExecutor`. First met in C08. See also [subinterpreter](#subinterpreter), [cross interpreter data](#cross-interpreter-data).
...
**Startup split in half, because the first half cannot import anything.**

Core initialisation builds the runtime, the main interpreter and the built in types, and it has to work with no import system, no `sys.path` and no encodings. Main initialisation is everything that needs those, including computing `sys.path` itself. The split is why a configuration error can be reported as a fatal error with a plain C string rather than as a Python exception: at that point there is nothing to raise.

Also written `pyinit_core`, `pyinit_main`, `Py_InitializeFromConfig`. First met in R01. See also [path configuration](#path-configuration), [safe path](#safe-path). In the source: [`Python/pylifecycle.c:1609-1639@v3.15.0rc1#Py_InitializeFromConfig`](https://github.com/python/cpython/blob/v3.15.0rc1/Python/pylifecycle.c#L1609-L1639).

### path configuration

**The set of paths worked out at startup, of which sys.path is the visible part.**
...
Also written `Modules/getpath.py`, `_PyConfig_InitPathConfig`. First met in R01. See also [two phase initialisation](#two-phase-initialisation), [safe path](#safe-path). In the source: [`Modules/getpath.c:855-894@v3.15.0rc1#_PyConfig_InitPathConfig`](https://github.com/python/cpython/blob/v3.15.0rc1/Modules/getpath.c#L855-L894).

### safe path

**The switch that stops the script's own directory going on the front of sys.path.**
...
Also written `-P`, `PYTHONSAFEPATH`, `config->safe_path`. First met in R01. See also [path configuration](#path-configuration). In the source: [`Modules/main.c:696-728@v3.15.0rc1#safe_path`](https://github.com/python/cpython/blob/v3.15.0rc1/Modules/main.c#L696-L728).

### module spec

**The answer a finder gives back, describing how a module is to be loaded.**

A `ModuleSpec` carries the module's full name, the loader that will run its body, an origin string saying where it came from, and `submodule_search_locations`, which is what makes a module a package. Every module you can reach has one on its `__spec__` attribute, so you can ask any module in a running program which of the finders answered for it and what it loaded from.

Also written `ModuleSpec`, `__spec__`, `spec.origin`. First met in R03. See also [meta path finder](#meta-path-finder), [path entry finder](#path-entry-finder). In the source: [`Lib/importlib/_bootstrap.py:605-640@v3.15.0rc1#ModuleSpec`](https://github.com/python/cpython/blob/v3.15.0rc1/Lib/importlib/_bootstrap.py#L605-L640).

### meta path finder

**Anything on sys.meta_path with a find_spec method, asked in order until one answers.**

The import machinery hands each finder the module name and, for a submodule, the parent package's `__path__`, and takes the first spec it gets back. A fresh interpreter starts with three: `BuiltinImporter` for modules compiled into the binary, `FrozenImporter` for modules baked in as bytecode, and `PathFinder` for everything reachable from `sys.path`. The list is an ordinary list and you can put your own class on the front of it.

Also written `sys.meta_path`, `find_spec`, `_find_spec`. First met in R03. See also [module spec](#module-spec), [path entry finder](#path-entry-finder), [module lock](#module-lock). In the source: [`Lib/importlib/_bootstrap.py:1198-1223@v3.15.0rc1#_find_spec`](https://github.com/python/cpython/blob/v3.15.0rc1/Lib/importlib/_bootstrap.py#L1198-L1223).

### path entry finder

**A finder that owns one entry of sys.path, cached in sys.path_importer_cache.**

`PathFinder` does not search directories itself. For each entry on `sys.path` it looks in `sys.path_importer_cache`, and on a miss it runs the entry past every hook in `sys.path_hooks` until one accepts it, storing the result, including `None` when nothing accepted. The usual answer is a `FileFinder`, which lists its directory once and reuses that listing until the directory's modification time changes.

Also written `sys.path_importer_cache`, `sys.path_hooks`, `FileFinder`. First met in R03. See also [meta path finder](#meta-path-finder), [module spec](#module-spec). In the source: [`Lib/importlib/_bootstrap_external.py:1211-1231@v3.15.0rc1#_path_importer_cache`](https://github.com/python/cpython/blob/v3.15.0rc1/Lib/importlib/_bootstrap_external.py#L1211-L1231).

### module lock

**One lock per module name, not one lock for importing.**

Two threads importing two different modules do not wait for each other, because each takes a lock keyed on the name it is loading. Two threads importing the same name do wait, and only one of them runs the body. The lock is taken after a check of `sys.modules` that runs without it, so the common case of importing something already loaded costs a dict lookup and never touches a lock at all.

Also written `_ModuleLock`, `_ModuleLockManager`, `_blocking_on`. First met in R03. See also [meta path finder](#meta-path-finder), [interpreter state](#interpreter-state). In the source: [`Lib/importlib/_bootstrap.py:226-240@v3.15.0rc1#_ModuleLock`](https://github.com/python/cpython/blob/v3.15.0rc1/Lib/importlib/_bootstrap.py#L226-L240).

### frozen module

**A module whose bytecode was compiled during CPython's own build and written into the binary.**

The build step compiles the source, marshals the code object and writes the bytes out as a C array in a generated header, so loading one is a read out of memory that was already mapped and no file is opened at all. Thirty three names are frozen in a stock 3.15 build. Three of them are the import system and cannot be switched off, and the rest are a startup speed decision that `-X frozen_modules=off` undoes.

Also written `_imp.is_frozen`, `-X frozen_modules`, `Python/frozen.c`. First met in R04. See also [import bootstrap](#import-bootstrap), [loader state](#loader-state), [marshal](#marshal). In the source: [`Python/frozen.c:74-79@v3.15.0rc1#bootstrap_modules`](https://github.com/python/cpython/blob/v3.15.0rc1/Python/frozen.c#L74-L79).

### import bootstrap

**The C code that loads the Python import system before there is an import system.**

`init_importlib` imports `_frozen_importlib` straight out of the frozen table, builds the `_imp` module by hand, and then calls `_install` on the module it has just loaded, handing `sys` and `_imp` in as arguments. That is why `Lib/importlib/_bootstrap.py` never writes `import sys` anywhere. The two modules it needs are passed to it and assigned to globals in `_setup`.

Also written `init_importlib`, `_setup`, `_install`. First met in R04. See also [frozen module](#frozen-module), [meta path finder](#meta-path-finder). In the source: [`Python/import.c:3389-3428@v3.15.0rc1#init_importlib`](https://github.com/python/cpython/blob/v3.15.0rc1/Python/import.c#L3389-L3428).

### loader state

**A free field on a module spec that a loader can use to remember whatever it likes.**

`ModuleSpec` sets `loader_state` to `None` and never looks at it again, so the field belongs to whichever loader filled the spec in. `FrozenImporter` puts a small namespace there holding the path the module was compiled from and the name it was frozen under, which is how `os.__file__` ends up pointing at a file that the import itself never opened.

Also written `spec.loader_state`, `origname`, `_fix_up_module`. First met in R04. See also [module spec](#module-spec), [frozen module](#frozen-module). In the source: [`Lib/importlib/_bootstrap.py:1106-1133@v3.15.0rc1#find_spec`](https://github.com/python/cpython/blob/v3.15.0rc1/Lib/importlib/_bootstrap.py#L1106-L1133).

### module alias

**A frozen entry that carries the name its source file really had.**

`importlib._bootstrap` is frozen under the name `_frozen_importlib`, because the interpreter needs it long before the `importlib` package exists as anything you could import. A table in `Python/frozen.c` maps one name to the other, and `_imp.find_frozen` hands the original back, so the loader can fix the module up afterwards and both names end up meaning the same module object.

Also written `aliases`, `is_alias`, `__origname__`. First met in R04. See also [frozen module](#frozen-module), [import bootstrap](#import-bootstrap). In the source: [`Python/frozen.c:127-137@v3.15.0rc1#aliases`](https://github.com/python/cpython/blob/v3.15.0rc1/Python/frozen.c#L127-L137).

### lazy import

**An import statement that binds the name now and loads the module later, or never.**

`lazy import json` is new in 3.15. It compiles to the same `IMPORT_NAME` opcode as a plain import, with two spare bits of the argument saying which kind this is, and the finding, reading and running of the module happen the first time something reads the name back. It is only allowed at module scope and not inside a `try` block, and the symbol table is what refuses the other spellings.

Also written `lazy import`, `lazy from`, `__lazy_modules__`. First met in R05. See also [import placeholder](#import-placeholder), [global import lock](#global-import-lock), [soft keyword](#soft-keyword). In the source: [`Grammar/python.gram:227-236@v3.15.0rc1#import_name`](https://github.com/python/cpython/blob/v3.15.0rc1/Grammar/python.gram#L227-L236).

### import placeholder

**The five field object a lazy import binds instead of a module.**
...
Also written `PyLazyImportObject`, `sys.lazy_modules`, `resolve`. First met in R05. See also [lazy import](#lazy-import), [global import lock](#global-import-lock). In the source: [`Include/internal/pycore_lazyimportobject.h:17-25@v3.15.0rc1#PyLazyImportObject`](https://github.com/python/cpython/blob/v3.15.0rc1/Include/internal/pycore_lazyimportobject.h#L17-L25).

### global import lock

**One recursive mutex per interpreter, taken while a lazy import resolves.**

An ordinary import takes a lock keyed on the module name, so two threads loading two different modules never wait for each other. Waking a placeholder does not use that lock. It takes the interpreter wide one and holds it for the whole resolution including the module body, which the source calls serialising reification. On a free threaded build that is the difference between deferred imports scaling across threads and not.

Also written `_PyImport_AcquireLock`, `IMPORT_LOCK`, `_PyRecursiveMutex`. First met in R05. See also [module lock](#module-lock), [import placeholder](#import-placeholder). In the source: [`Python/import.c:150-158@v3.15.0rc1#_PyImport_AcquireLock`](https://github.com/python/cpython/blob/v3.15.0rc1/Python/import.c#L150-L158).

### interpreter finalisation

**Everything that happens after your last line, laid out in one C function.**

`_Py_Finalize` runs the whole ending in a fixed order: the pre finalization calls, which join your non daemon threads and run the atexit callbacks, then the flag that says shutdown has begun, then the other thread states go, then one garbage collection, then the modules are torn down and cleared, then the interpreter and the runtime themselves. Nothing in the second half is a normal place to be running Python.

Also written `_Py_Finalize`, `Py_FinalizeEx`, `make_pre_finalization_calls`. First met in R08. See also [atexit callback](#atexit-callback), [finalizing flag](#finalizing-flag), [daemon thread](#daemon-thread). In the source: [`Python/pylifecycle.c:2380-2419@v3.15.0rc1#_Py_Finalize`](https://github.com/python/cpython/blob/v3.15.0rc1/Python/pylifecycle.c#L2380-L2419).

### atexit callback

**A function you register to run at the end, while Python still works properly.**

`atexit.register` inserts at the front of one list, so callbacks come back in the reverse of the order you registered them. The list is copied before it is walked and then emptied, which is why a callback that registers another one during shutdown is never called. If a callback raises, the error is printed as an ignored exception and the process exit status does not change.

Also written `atexit.register`, `atexit._ncallbacks`, `atexit_callfuncs`. First met in R08. See also [interpreter finalisation](#interpreter-finalisation), [finalizing flag](#finalizing-flag). In the source: [`Modules/atexitmodule.c:102-141@v3.15.0rc1#atexit_callfuncs`](https://github.com/python/cpython/blob/v3.15.0rc1/Modules/atexitmodule.c#L102-L141).

### finalizing flag

**The bit that says shutdown has started, readable as sys.is_finalizing().**

It is set after the atexit callbacks have already run, so a callback sees `False` and a `__del__` on a module global sees `True`. That one bit is the honest test for whether you are in the part of shutdown where the usual rules have stopped applying. By then `sys.modules` has been emptied and any import, even of a module you already imported, raises `ImportError` saying `sys.meta_path is None`.

Also written `sys.is_finalizing`, `_PyInterpreterState_SetFinalizing`, `interp->finalizing`. First met in R08. See also [interpreter finalisation](#interpreter-finalisation), [atexit callback](#atexit-callback). In the source: [`Python/pylifecycle.c:2380-2419@v3.15.0rc1#_PyInterpreterState_SetFinalizing`](https://github.com/python/cpython/blob/v3.15.0rc1/Python/pylifecycle.c#L2380-L2419).
...
**The C functions and macros an extension may call to talk to the interpreter.**

It is not one list but three, and which one you get is decided before you include anything. `Include/` is open to everybody, `Include/cpython/` needs `Py_LIMITED_API` to be undefined, and `Include/internal/` stops the compiler with an error unless you define `Py_BUILD_CORE`. More than half the header lines are in the third directory, the one no extension is meant to open.

Also written `Python.h`. First met in R06. See also [limited API](#limited-api), [PyAPI_FUNC](#pyapi_func), [stable ABI](#stable-abi). In the source: [`Doc/c-api/stable.rst:21-35@v3.15.0rc1`](https://github.com/python/cpython/blob/v3.15.0rc1/Doc/c-api/stable.rst#L21-L35).

### limited API

**The subset of the C API you get when you define Py_LIMITED_API before Python.h.**

It takes away about a quarter of the public functions, which is the part people notice, and all of the struct layouts, which is the part that costs. With no fields to read, `Py_TYPE` stops being a pointer dereference and becomes a function call, and `Py_DECREF` becomes a call to `_Py_DecRef`. What you buy with that is one built file that keeps working across releases.

Also written `Py_LIMITED_API`. First met in R06. See also [C API](#c-api), [stable ABI](#stable-abi). In the source: [`Include/object.h:124-126@v3.15.0rc1#_Py_OPAQUE_PYOBJECT`](https://github.com/python/cpython/blob/v3.15.0rc1/Include/object.h#L124-L126).

### PyAPI_FUNC

**The macro that marks a declaration as a symbol other binaries may link against.**

It expands to a compiler attribute asking for default visibility, so a declaration that uses it leaves the shared library and a plain `extern` does not. The internal headers use both spellings, often two lines apart, and the choice is deliberate: CPython's own bundled extensions are separate shared libraries that need to reach back in, and a comment above each export usually names which one.

First met in R06. See also [C API](#c-api). In the source: [`Include/exports.h:88-93@v3.15.0rc1#PyAPI_FUNC`](https://github.com/python/cpython/blob/v3.15.0rc1/Include/exports.h#L88-L93).

### stable ABI

**An extension build that keeps working across releases, marked abi3 in its name.**

The tiers are a compile time idea and none of them survive into a built extension. The file name does. An interpreter decides what it is willing to load from the suffix alone, so `something.abi3.so` will load on many releases while `something.cpython-315-darwin.so` will load on exactly one. Python 3.15 adds `abi3t` for free threaded builds.

Also written abi3, abi3t. First met in R06. See also [limited API](#limited-api), [abi flags](#abi-flags), [extension module](#extension-module).

### extension module

**A module that arrives as a compiled shared library rather than as Python.**

Everything about how it loads is decided in two steps. First the finder looks at the file name and decides whether the tag in it is one this build accepts, which costs one stat call and happens before anything is opened. Then, if the extension was built against 3.15 or later, the interpreter compares the ABI struct it carries against itself, by which point the extension's own code has already run.

Also written `.so`, `.pyd`. First met in R07. See also [stable ABI](#stable-abi), [SOABI](#soabi), [PyABIInfo](#pyabiinfo). In the source: [`Python/dynload_shlib.c:39-60@v3.15.0rc1#_PyImport_DynLoadFiletab`](https://github.com/python/cpython/blob/v3.15.0rc1/Python/dynload_shlib.c#L39-L60).

### module state

**A struct hanging off a module object, one copy per module rather than per process.**

A module definition with a positive `m_size` gets that many bytes allocated alongside every module object, reachable with `PyModule_GetState`. It is where an extension keeps what a C file would otherwise keep in statics, which matters because statics are shared by every interpreter in the process and module state is not. A type built from a spec can find its own module's state again through `PyType_GetModuleByDef`.

Also written `m_size`, `PyModule_GetState`, `PyType_GetModuleByDef`. First met in R09. See also [extension module](#extension-module), [two phase initialisation](#two-phase-initialisation), [subinterpreter](#subinterpreter). In the source: [`Objects/moduleobject.c:1027-1035@v3.15.0rc1#PyModule_GetState`](https://github.com/python/cpython/blob/v3.15.0rc1/Objects/moduleobject.c#L1027-L1035).

### SOABI

**The tag a build stamps into the file names of the extensions it compiles.**

It reads like `cpython-315t-darwin`, and it is three separate facts glued together: the implementation, the version with a `t` on the end if the build has no global interpreter lock, and the platform. Change any one of them and this interpreter will not even look at the file. The value is in `sysconfig`, and the suffix built from it is the first entry of `importlib.machinery.EXTENSION_SUFFIXES`.

Also written `EXT_SUFFIX`. First met in R07. See also [extension module](#extension-module), [abi flags](#abi-flags).

### PyABIInfo

**Twelve bytes an extension carries saying which ABI it was built against.**

Two version bytes for the struct itself, sixteen flag bits for stable, GIL, free threaded and internal, and two packed version numbers. `PyABIInfo_Check` compares it against the running interpreter and raises `ImportError` when they disagree, which is the only ABI check CPython does at all. It is new in 3.15, it is opt in, and the extension's own code has already run by the time it happens.

Also written `PyABIInfo_Check`, `Py_mod_abi`. First met in R07. See also [stable ABI](#stable-abi), [extension module](#extension-module). In the source: [`Include/modsupport.h:86-101@v3.15.0rc1#PyABIInfo`](https://github.com/python/cpython/blob/v3.15.0rc1/Include/modsupport.h#L86-L101).
